...
- 2026-06-01T09:47:00-07:00: Added a minimal fast Vitest lane focused on high-breakage frontend logic. `package.json` now has `npm test` -> `vitest run`; `vitest.config.js` scopes discovery to `test/**/*.test.js` with `environment: 'node'` to avoid pulling browser/jsdom dependencies into the default quick lane.
- 2026-06-01T09:47:00-07:00: Refactored `scripts/fetch-data.js` to be safely importable in tests by moving `DASHBOARD_OWNER` validation into `main()` and guarding execution with `if (require.main === module)`. Exported pure heuristics for direct tests (`normalizeLabels`, `issuePriority`, `buildPriorityIssues`, `summarizeNextSteps`, `computeNextStepState`, etc.) without changing production CLI behavior.
...
- 2026-06-27T22:55:35-07:00: PR #48 review fixes. (1) Reuse already-computed `closed` Set in `renderRepos()` instead of re-calling `getClosedRepos()` for the Kanban filter — avoids extra JSON parse and mid-render inconsistency. (2) `_renderTopPickBar` now uses `${topPick.url}` for the link href — single source of truth, no drift risk if the URL changes. (3) Exported `_isDependabotPR` and `_findTopPick` via `GHD.KanbanStrip`; added 14 new unit tests covering Dependabot detection (all author formats, case-insensitive), null inputs, sort-by-count precedence, mixed-author counting, and url value. Suite grew from 68 → 82 passing.
